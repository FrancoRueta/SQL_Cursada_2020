...
CREATE TABLE `compania_envios` (
	...
	`NOMBRE_COMPANIA` VARCHAR(50) NOT NULL DEFAULT '' COLLATE 'latin1_swedish_ci',
	`TELEFONO` INT(11) NOT NULL DEFAULT '0',
...
CREATE TABLE `empleado` (
	...
	`FOTO` VARCHAR(50) NOT NULL DEFAULT '0' COLLATE 'latin1_swedish_ci',
...
CREATE TABLE `pedido` (
	...
	`ID_COMPANIA_ENVIOS` INT(10) NOT NULL,
	...
	`COD_POSTAL_DESTINATARIO` INT(10) NOT NULL DEFAULT '0',
	...
	CONSTRAINT `FK_pedido_compania_envios_2` FOREIGN KEY (`ID_COMPANIA_ENVIOS`) REFERENCES `tp_bdd_28_9`.`compania_envios` (`ID_COMPANIA_ENVIOS`) ON UPDATE NO ACTION ON DELETE NO ACTION,
...
/*Listar todos los PEDIDOS (Nombre de Producto y Cantidad) solicitados por los clientes listando de ellos Nombre, Cargo y País. Y de los empleados
 ...
 SELECT pedido.ID_PEDIDO,cliente.NOMBRE_CONTACTO,cliente.CARGO_CONTACTO,cliente.CIUDAD,empleado.APELLIDOS,empleado.NOMBRE,empleado.CARGO,empleado.CIUDAD,empleado.PAIS
 FROM pedido,cliente,empleado
...
/*b.0 Listar todos los datos de los clientes que viven en la ciudad de Mendoza y de los empleados que han atendido a dichos clientes pero que viven en la ciudad de Córdoba.*/
 ...
 WHERE cliente.CIUDAD = 'buenos aires'
 AND empleado.CIUDAD = 'cordoba'
 AND empleado.ID_EMPLEADO = pedido.ID_PEDIDO
...
SELECT AVG(empleado.COD_POSTAL) AS 'Edad promedio'
FROM pedido,empleado
WHERE empleado.CIUDAD = 'salta'
AND empleado.NOMBRE_CONTACTO LIKE 'J%'
